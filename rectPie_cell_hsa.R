...
df_cell <- df_f[which(grepl(df_f$celltissue, pattern = "cell lines")),]
##################
...
g_rec <-
  ...
  scale_fill_manual(breaks = sb_1$celltissue,
                    values = rev(c(pal_d3(palette = "category20")(20)[1:7])),
                    labels = paste(sb_1$celltissue," (",sb_1$frac,")")) +
  #  geom_text_repel(data = sb_1, 
  #                  aes(x=3.5,y=(ymax-ymin)/2,label=frac,hjust=0,
  #                                   colour="black",family="serif"),size=4) +
  guides(fill = guide_legend(order = 1, nrow = 7, title = "Cell line",reverse = T))
g_rec
g_sb_03 <-
  g_rec +
  coord_polar(theta = "y",direction = -1) +
  ggtitle(label = "hsa eccDNA distribution by cell line") +
  theme_classic() +
  theme(legend.position = c(1.25,0.3),
  ...
  annotate("text", label = "hsa (422734)\n---------------\nDerived from cell \nlines", 
           x=0, y=0.5, family="serif", size=6)
g_sb_03
#ggsave("rectPie_cell_hsa.jpg",dpi=800)
ggsave(g_sb_03,filename = "AI/rectPie_cell_hsa.pdf",units = "cm", width = 24, height = 15)
