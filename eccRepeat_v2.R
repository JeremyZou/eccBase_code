...
p_3 <- ggplot() +
       ...
       scale_y_continuous(limits = c(0,20),expand = c(0.04, 0), breaks = seq(0,20,by=4),
       ...
       xlab(expression(Number~of~eccDNA~repeats)) +ylab("Log2(Number of eccDNAs + 1)")
p_3
ggsave(plot = p_3, filename = "repeat_histogram_line_hsa_v2.pdf", units = "cm",width = 8, height = 6)
dev.off()
#开始作图：小鼠的柱状图 + 频率折线图
...
p_4 <- ggplot() +
       ...
       xlab(expression(Number~of~eccDNA~repeats)) +ylab("Log2(Number of eccDNAs + 1)")
p_4
ggsave(plot = p_4, filename = "repeat_histogram_line_mmu_v2.pdf",units = "cm", width = 8, height = 6)
dev.off()
