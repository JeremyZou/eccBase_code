...
p_3x <- ggplot(dataHis_1,aes(x=Length)) +
  ...
  xlab(expression(eccDNA~size~("×"~10^6~bp))) +ylab("Log2(Number of eccDNAs + 1)")
p_3x
ggsave(plot = p_3x, filename = "fullLength_histogram_bar_hsa_v2.pdf",units="cm",width = 8,height = 6)
dev.off()
#################################################################################################################
#################################################################################################################
#################################################################################################################
...
p_4x <- ggplot(aa_2m,aes(x=Length)) +
        ...
        xlab(expression(eccDNA~size~(bp))) +ylab("Log2(Number of eccDNAs + 1)")
p_4x
getwd()
ggsave(plot = p_4x, filename = "fullLength_histogram_bar_mmu_v2.pdf",units="cm",width = 8,height = 6)
dev.off()
#3、开始作图:bar：柱状图
jpeg(filename = "fullLength_bar_hsa_1.jpg",res = 800,units = "cm",width = 9,height = 6)
